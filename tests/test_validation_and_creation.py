...
from clasp_diagrams.validators import ClaspDiagramCreationError
...
def test_none_matrix_raises_value_error():
    with pytest.raises(ClaspDiagramCreationError, match="matrix argument is None"):
...
def test_non_unique_heights_raises_value_error():
    ...
    with pytest.raises(ClaspDiagramCreationError, match="The heights are invalid"):
        ClaspDiagram.from_matrix(matrix=bad_matrix)
...
def test_invalid_sign_raises_value_error():
    ...
    with pytest.raises(ClaspDiagramCreationError, match="Invalid sign encountered in"):
        ClaspDiagram.from_matrix(matrix=bad_matrix)
...
def test_invalid_end_point_raises_value_error():
    ...
    with pytest.raises(ClaspDiagramCreationError, match="Invalid start and endpoint in"):
        ClaspDiagram.from_matrix(matrix=bad_matrix)
...
def test_start_points_not_strictly_increasing_raises_value_error():
    ...
    with pytest.raises(ClaspDiagramCreationError, match="Invalid order of the start points:"):
        ClaspDiagram.from_matrix(matrix=bad_matrix)
...
def test_invalid_points_raises_value_error():
    ...
    with pytest.raises(ClaspDiagramCreationError, match="Invalid start/end points"):
        ClaspDiagram.from_matrix(matrix=bad_matrix)
...
def test_none_array_raises_value_error():
    with pytest.raises(ClaspDiagramCreationError, match="array argument is None"):
...
def test_validate_odd_length_raises_value_error():
    with pytest.raises(ClaspDiagramCreationError, match="Array must contain an even number of ChordForArray instances."):
...
def test_validate_non_double_occurrence_raises():
    ...
    with pytest.raises(ClaspDiagramCreationError, match="do not appear exactly twice"):
...
def test_validate_sign_raises():
    ...
    with pytest.raises(ClaspDiagramCreationError, match="Invalid sign encountered"):
        ClaspDiagram.from_array(arr)
...
def test_validate_chord_idx_raises():
    ...
    arr = [chord1, chord1, chord2, chord2]
    with pytest.raises(ClaspDiagramCreationError, match="Invalid chord_idx encountered"):
        ClaspDiagram.from_array(arr)
...
def test_validate_height_raises():
    ...
    chord2 = ChordForArray(2, '+', 1)
    arr = [chord1, chord1, chord2, chord2]
    with pytest.raises(ClaspDiagramCreationError, match="Invalid height encountered"):
        ClaspDiagram.from_array(arr)
...
def test_validate_chord_idx_raises2():
    ...
    arr = [chord1, chord1, chord2, chord2]
    with pytest.raises(ClaspDiagramCreationError, match="Invalid ordering of chord idxs:"):
        ClaspDiagram.from_array(arr)
...
def test_move_delegator_raises_error():
    ...
    with pytest.raises(ValueError, match="Invalid move_num"):
        clasp.move(move_num=-99)

def test_move_delegator():
    clasp = ClaspDiagram.from_matrix(matrix=(ChordForMatrix(0, 1, '+', 1),
                                             ChordForMatrix(2, 3, '+', 2)))

    clasp.move(move_num=1, i=1, j=2)
    clasp.move(move_num=2)
    clasp.move(move_num=-2)
    clasp.move(move_num=3, i=2)
    clasp.move(move_num=-3, after_point=1, new_sign='+', new_height=2)
